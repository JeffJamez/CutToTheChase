...
```
~/Library/Containers/no.mifi.CutToTheChase-mac/Data/Library/Application Support
```

### Starting over fresh

```bash
rm -rf ~/Library/Containers/no.mifi.CutToTheChase-mac
```
...
- _If Store-only hotfix release_
  - `git checkout stores`
  - `npm version patch`
- _If normal GitHub-first release_
...
- _If Stores-only hotfix release_
  - `git checkout master`
  - `git merge stores`
- Bump [snap version](https://snapcraft.io/CutToTheChase/releases)
...
```bash
yarn pack-mas-dev
cat dist/mas-dev-arm64/CutToTheChase.app/Contents/Info.plist
```
...
#cp licenses.txt CutToTheChase.mifi.no/public/
```
