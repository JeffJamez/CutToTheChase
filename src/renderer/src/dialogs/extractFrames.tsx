...
export async function askExtractFramesAsImages({
  segmentsNumFrames,
  plural,
  fps,
}: {
  segmentsNumFrames: number;
  plural: boolean;
  fps: number;
}) {
  const { value: captureChoice } = await getSwal().Swal.fire<string>({
    text: i18n.t(
      plural
        ? "Extract frames of the selected segments as images"
        : "Extract frames of the current segment as images",
    ),
    icon: "question",
    input: "radio",
    inputValue: "thumbnailFilter",
    showCancelButton: true,
    customClass: { input: "swal2-CutToTheChase-radio" },
    inputOptions: {
  ...
  if (captureChoice === "thumbnailFilter") {
    const { value } = await getSwal().Swal.fire({
      ...
      input: "text",
      inputLabel: i18n.t(
        "Enter the max number of seconds between each image (decimal)",
      ),
      inputValue: 5,
      showCancelButton: true,
    });
  ...
  if (captureChoice === "selectNthSec" || captureChoice === "selectNthFrame") {
    ...
    if (captureChoice === "selectNthFrame") {
      const { value } = await getSwal().Swal.fire({
        ...
        input: "number",
        inputLabel: i18n.t(
          "Enter the number of frames between each image (integer)",
        ),
        inputValue: 30,
        showCancelButton: true,
      });
      ...
      const { value } = await getSwal().Swal.fire({
        ...
        input: "text",
        inputLabel: i18n.t(
          "Enter the number of seconds between each image (decimal)",
        ),
        inputValue: 5,
        showCancelButton: true,
      });
  ...
  if (captureChoice === "selectScene") {
    const { value } = await getSwal().Swal.fire({
      ...
      input: "text",
      inputLabel: i18n.t(
        "Enter a decimal number between 0 and 1 (sane values are 0.3 - 0.5)",
      ),
      inputValue: "0.4",
      showCancelButton: true,
    });
  ...
  if (estimatedMaxNumFiles > 1000) {
    const { isConfirmed } = await getSwal().Swal.fire({
      icon: "warning",
      text: i18n.t(
        "Note that depending on input parameters, up to {{estimatedMaxNumFiles}} files may be produced!",
