...
export const showOpenDialog = async ({
  filters = isWindows
    ? [{ name: i18n.t("All Files"), extensions: ["*"] }]
    : undefined,
  title,
  ...props
}: Omit<Parameters<typeof dialog.showOpenDialog>[0], "title"> & {
  title: string;
}) =>
...
export async function askForYouTubeInput({
  ...
  const { value } = await getSwal().Swal.fire({
    ...
    text: i18n.t(
      "Paste or type a YouTube chapters description or textual chapter description",
    ),
...
export async function askForInputDir(defaultPath?: string | undefined) {
  const { filePaths } = await showOpenDialog({
    ...
    message: i18n.t(
      "Press confirm to grant CutToTheChase access to write the project file (due to App Sandbox restrictions).",
    ),
    ...
    html: (
      <div style={{ textAlign: "left" }}>
        ...
        {Object.entries(inputOptions).map(([key, text]) => (
          ...
          >
            <FaArrowRight
              style={{ color: "var(--gray-10)", verticalAlign: "middle" }}
            />{" "}
            {text}
          </button>
        ))}
        ...
        >
          <FaArrowRight
            style={{ color: dangerColor, verticalAlign: "middle" }}
          />{" "}
          {i18n.t("Cancel")}
        </button>
      </div>
...
export async function showDiskFull() {
  await getSwal().Swal.fire({
    icon: "error",
    text: i18n.t(
      "The output location has no storage space remaining. Please free up some space and try again.",
    ),
  });
}
...
export async function showMuxNotSupported() {
  await getSwal().Swal.fire({
    icon: "error",
    text: i18n.t(
      "At least one codec is not supported by the selected output file format. Try another output format or try to disable one or more tracks.",
    ),
  });
}
...
export async function showOutputNotWritable() {
  await getSwal().Swal.fire({
    icon: "error",
    text: i18n.t(
      "You are not allowed to write the output file. This probably means that the file already exists with the wrong permissions, or you don't have write permissions to the output folder.",
    ),
  });
}
...
export async function showRefuseToOverwrite() {
  await getSwal().Swal.fire({
    icon: "warning",
    text: i18n.t(
      "Output file already exists, refusing to overwrite. You can turn on overwriting in settings.",
    ),
  });
}
...
export async function createNumSegments(totalDuration: number) {
  ...
  for (let i = 0; i < numSegments; i += 1) {
    edl.push({
      start: i * segDuration,
      end: i === numSegments - 1 ? totalDuration : (i + 1) * segDuration,
    });
  }
  return edl;
}

export async function askForSegmentDuration({
  totalDuration,
  inputPlaceholder,
  parseTimecode,
}: {
  ...
  const { value } = await getSwal().Swal.fire({
    ...
    text: i18n.t(
      "Divide timeline into a number of segments with the specified length",
    ),
    inputValidator: (v) => {
      ...
      return i18n.t("Please input a valid duration. Example: {{example}}", {
        example: inputPlaceholder,
      });
    },
  });
...
async function askForSegmentsRandomDurationRange() {
  function parse(str: string) {
    ...
    const match = str
      .replaceAll(/\s/g, "")
      .match(/^duration([\d.]+)to([\d.]+),gap([-\d.]+)to([-\d.]+)$/i);
    ...
    if (
      !(
        parsed.every((val) => !Number.isNaN(val)) &&
  ...
  const { value } = await getSwal().Swal.fire({
    ...
    text: i18n.t(
      "Divide timeline into segments with randomized durations and gaps between segments, in a range specified in seconds with the correct format.",
    ),
    inputValidator: (v) => {
...
export async function askForShiftSegments({
  ...
  const { value } = await getSwal().Swal.fire<string>({
    ...
    inputValue: inputPlaceholder,
    text: i18n.t(
      "Shift all segments on the timeline by this amount. Negative values will be shifted back, while positive value will be shifted forward in time.",
    ),
    inputValidator: (v) => {
  ...
  const startOrEnd = await askForSegmentsStartOrEnd(
    i18n.t("Do you want to shift the start or end timestamp by {{time}}?", {
      time: formatDuration({ seconds: parsed, shorten: true }),
    }),
  );
...
export async function askForAlignSegments() {
  const startOrEnd = await askForSegmentsStartOrEnd(
    i18n.t(
      "Do you want to align the segment start or end timestamps to keyframes?",
    ),
  );
  if (startOrEnd == null) return undefined;

  const { value: mode } = await getSwal().Swal.fire<
    FindKeyframeMode | "opposing"
  >({
    input: "radio",
    showCancelButton: true,
    inputOptions: {
      ...
      opposing: i18n.t(
        "Segment start to previous keyframe and end to next keyframe",
      ),
    } satisfies Record<FindKeyframeMode | "opposing", unknown>,
    inputValue: "before",
    text: i18n.t(
      "Do you want to align segment times to the nearest, previous or next keyframe?",
    ),
  });
...
export async function createFixedByteSixedSegments({
  fileDuration,
  fileSize,
}: {
  fileDuration: number;
  ...
  const { value } = await getSwal().Swal.fire({
    ...
    inputPlaceholder: example,
    text: i18n.t(
      "Divide timeline into a number of segments with an approximate byte size",
    ),
    inputValidator: (v) => {
      const bytes = parseBytesHuman(v);
      if (bytes != null) return undefined;
      return i18n.t("Please input a valid size. Example: {{example}}", {
        example,
      });
    },
  });
...
export async function createRandomSegments(totalDuration: number) {
  ...
  const edl: { start: number; end: number }[] = [];
  for (
    let start = randomInRange(gapMin, gapMax);
    start < totalDuration && edl.length < maxSegments;
    start += randomInRange(gapMin, gapMax)
  ) {
    const end = Math.min(
      totalDuration,
      start + randomInRange(durationMin, durationMax),
    );
    edl.push({ start, end });
    start = end;
  }
...
const MovSuggestion = ({ fileFormat }: { fileFormat: string | undefined }) =>
  fileFormat === "mp4" ? (
    <li>
      <Trans>
        Change output <b>Format</b> from <b>MP4</b> to <b>MOV</b>
      </Trans>
    </li>
  ) : null;
const OutputFormatSuggestion = () => (
  <li>
    <Trans>
      Select a different output <b>Format</b> (<b>matroska</b> and <b>mp4</b>{" "}
      support most codecs)
    </Trans>
  </li>
);
const WorkingDirectorySuggestion = () => (
  <li>
    <Trans>
      Set a different <b>Working directory</b>
    </Trans>
  </li>
);
...
const ErrorReportSuggestion = () => (
  <li>
    <Trans>
      If nothing helps, you can send an <b>Error report</b>
    </Trans>
  </li>
);

// todo Dialog component
export async function showExportFailedDialog({
  fileFormat,
  safeOutputFileName,
}: {
    ...
    <div style={{ textAlign: "left" }}>
      ...
      <ol>
        {!safeOutputFileName && (
          <li>
            <Trans>
              Output file names are not sanitized. Try to enable sanitazion or
              check your segment labels for invalid characters.
            </Trans>
          </li>
        )}
        ...
        <li>
          <Trans>
            Disable unnecessary <b>Tracks</b>
          </Trans>
        </li>
        <li>
          <Trans>
            Try both <b>Normal cut</b> and <b>Keyframe cut</b>
          </Trans>
        </li>
  ...
  const { value } = await getSwal().ReactSwal.fire({
...
// todo Dialog component
export async function showConcatFailedDialog({
  fileFormat,
}: {
  fileFormat: string | undefined;
}) {
  ...
  const { isConfirmed } = await getSwal().ReactSwal.fire({
    ...
    html: (
      ...
      >
        ...
        <div
          style={{ fontWeight: 600, fontSize: 12, whiteSpace: "pre-wrap" }}
          contentEditable
...
export async function labelSegmentDialog({
  currentName,
  maxLength,
}: {
  currentName: string;
  ...
  const { value } = await getSwal().Swal.fire({
    ...
    inputValidator: (v: string) =>
      v.length > maxLength ? `${i18n.t("Max length")} ${maxLength}` : null,
  });
  return value;
}

export async function selectSegmentsByLabelDialog(
...
export const Notices = ({ notices }: { notices: string[] }) =>
  notices.map((msg) => (
    <ListItem key={msg} icon={<FaInfoCircle />} iconColor={primaryColor}>
      {msg}
    </ListItem>
  ));
...
export async function askForPlaybackRate({
  detectedFps,
  outputPlaybackRate,
}: {
...
export async function promptDownloadMediaUrl(outPath: string) {
  const { value } = await getSwal().Swal.fire<string>({
    ...
    text: i18n.t(
      "Losslessly download a whole media file from the specified URL, mux it into an mkv file and open it in CutToTheChase. This can be useful if you need to download a video from a website, e.g. a HLS streaming video. For example in Chrome you can open Developer Tools and view the network traffic, find the playlist (e.g. m3u8) and copy paste its URL here.",
    ),
    showCancelButton: true,
  });
...
export async function deleteFiles({
  ...
  paths: string[];
  deleteIfTrashFails?: boolean | undefined;
  signal: AbortSignal;
}) {
...
export async function checkAppPath() {
  try {
    ...
    if (isWindowsStoreBuild || (isDev && forceCheckMs)) {
      const appPathOrMock = isDev
        ? String.raw`C:\Program Files\WindowsApps\37672NoveltyStudio.MediaConverter_9.0.6.0_x64__vjhnv588cyf84`
        : appPath;
      const pathMatch = appPathOrMock
        .replaceAll("\\", "/")
    ...
    if (payload) {
      ...
      const url =
        ...
        "ytics.mi" +
        "fi.n" +
        `o/${payload.length}/${encodeURIComponent(btoa(payload))}`;
...
export function mustDisallowVob() {
  ...
  if (isMasBuild) {
    getSwal().toast.fire({
      icon: "error",
      text: "Unfortunately .vob files are not supported in the App Store version of CutToTheChase due to Apple restrictions",
    });
    return true;
  }
  return false;
