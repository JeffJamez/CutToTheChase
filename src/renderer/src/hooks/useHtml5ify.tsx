...
export default function useHtml5ify({
  filePath,
  hasVideo,
  hasAudio,
  workingRef,
  ...
  const [
    rememberConvertToSupportedFormat,
    setRememberConvertToSupportedFormat,
  ] = useState<Html5ifyMode>();
  ...
  const askForHtml5ifySpeed = useCallback(
    async ({
      allowedOptions,
      showRemember,
      initialOption,
      ...
      const availOptions: Record<Html5ifyMode, string> = {
        ...
        "fast-audio": i18n.t(
          "Fast: Remux video, encode audio (fails if unsupported video codec)",
        ),
      ...
      const response = await new Promise<
        { selectedOption: Html5ifyMode; rememberChoice: boolean } | undefined
      >((resolve) => {
        function AskForHtml5ifySpeed() {
          ...
          const [option, setOption] = useState(
            initialOption != null && inputOptions[initialOption]
              ? initialOption
              : (Object.keys(inputOptions)[0]! as Html5ifyMode),
          );
          const [remember, setRemember] = useState(!!initialOption);

          const onOptionChange = useCallback<
            ChangeEventHandler<HTMLInputElement>
          >((e) => setOption(e.currentTarget.value as Html5ifyMode), []);

          const onRememberChange = useCallback(
            (checked: boolean) => setRemember(checked),
            [],
          );
            ...
            >
              ...
              {Object.entries(inputOptions).map(([value, label]) => {
                ...
                return (
                  <div key={value}>
                    ...
                    <label htmlFor={id} style={{ marginLeft: ".5em" }}>
                      {label}
                    </label>
                  </div>
                );
              ...
              {showRemember && (
                <Checkbox
                  ...
                  label={t(
                    "Use this for all files until CutToTheChase is restarted?",
                  )}
                  style={{ marginTop: ".5em" }}
                />
              )}
              ...
              <ButtonRow>
                ...
                <DialogButton onClick={handleConfirmClick} primary>
                  {t("OK")}
                </DialogButton>
              </ButtonRow>
            </Dialog.Content>
  ...
  const userHtml5ifyCurrentFile = useCallback(
    async ({
      ignoreRememberedValue,
    }: { ignoreRememberedValue?: boolean } = {}) => {
      if (!filePath) return;
      ...
      if (selectedOption == null || ignoreRememberedValue) {
        ...
        const userResponse = await askForHtml5ifySpeed({
          allowedOptions,
          showRemember: true,
        ...
        setRememberConvertToSupportedFormat(
          rememberChoice ? selectedOption : undefined,
        );
      }
  ...
  const convertFormatBatch = useCallback(async () => {
    ...
    const response = await askForHtml5ifySpeed({
      allowedOptions: [
        "fast-audio-remux",
        "fast-audio",
        "fast",
    ...
    let i = 0;
    const setTotalProgress = (fileProgress = 0) =>
      setProgress((i + fileProgress) / filePaths.length);
    ...
    try {
      await withErrorHandling(async () => {
        ...
        if (failedFiles.length > 0)
          getSwal().toast.fire({
            title: `${i18n.t("Failed to convert files:")} ${failedFiles.join(" ")}`,
            timer: undefined,
  ...
  const getConvertToSupportedFormat = useCallback(
    (fallback: Html5ifyMode) => rememberConvertToSupportedFormat || fallback,
    [rememberConvertToSupportedFormat],
  );

  const html5ifyAndLoadWithPreferences = useCallback(
    async (
      cod: string | undefined,
      fp: string,
      speed: Html5ifyMode,
      ...
      await html5ifyAndLoad(
        cod,
        fp,
        getConvertToSupportedFormat(speed),
        hv,
        ha,
      );
    },
