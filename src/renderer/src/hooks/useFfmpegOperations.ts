...
function useFfmpegOperations({
  ...
  const concatFiles = useCallback(
    async ({
      ...
      try {
        ...
        // Keep this similar to CutToTheChaseSingle()
        const ffmpegArgs = [
          "-hide_banner",
  ...
  const CutToTheChaseSingle = useCallback(
    async ({
      ...
      const cutFromArgs = cuttingStart
        ? ["-ss", cutFromWithAdjustment.toFixed(5)]
        : [];
      ...
      const avoidNegativeTsArgs =
        cuttingStart && avoidNegativeTs && ssBeforeInput
          ? ["-avoid_negative_ts", String(avoidNegativeTs)]
          : [];
      ...
      const inputFilesArgs =
        copyFileStreamsFiltered.length > 1
          ? flatMap(copyFileStreamsFiltered, ({ path }) => [
              ...cutFromArgs,
              "-i",
              path,
              ...cutToArgs,
  ...
  const cutEncodeSmartPart = useCallback(
    async ({
      ...
      const ffmpegArgs = [
        ...
        "-ss",
        cutFrom.toFixed(5), // if we don't -ss before -i, seeking will be slow for long files, see https://github.com/mifi/lossless-cut/issues/126#issuecomment-1135451043
        "-i",
        filePath,
        "-ss",
  ...
  const cutMultiple = useCallback(
    async ({
      ...
      // This function will either call CutToTheChaseSingle (if no smart cut enabled)
      ...
      const cutSegment = async (
        ...
        const { CutToTheChaseFrom, segmentNeedsSmartCut } = await needsSmartCut(
          { path: filePath, desiredCutFrom, videoStream },
        );
        ...
        if (segmentNeedsSmartCut && CutToTheChaseFrom > cutTo) {
          console.log(
            "Segment is between two keyframes, cutting/encoding the whole segment",
            { desiredCutFrom, CutToTheChaseFrom, cutTo },
          );
          return cutEncodeWholePart();
        }
        ...
        try {
          ...
          const encodeCutToSafe = Math.max(
            ...
            CutToTheChaseFrom - frameDuration,
          );
