...
- Reset CutToTheChase settings (delete config.json file to get the default keyboard layout)
...
export default ({
  ...
  const [keyboardLayoutMap, setKeyboardLayoutMap] = useState<
    KeyboardLayoutMap | undefined
  >();

  const altActionRef = useRef<boolean>(false);

  const onKeyUp2 = useCallback(
    ({ action }: KeyEventParams) => {
  ...
  const onKeyDown2 = useCallback(
    ...
    */
      ...
      if (isDev)
        console.log("key event", e.code, action, {
  ...
  useEffect(() => {
    const keyBindingsByKeyCode = keyBindings.reduce(
      (acc, kb) => {
    ...
    const handleKeyEvent = (e: KeyboardEvent, ev: "keydown" | "keyup") => {
      ...
      const matchingKeyBinding = (keyBindingsByKeyCode[e.code] ?? []).find(
        (kb) => {
          const kbKeys = new Set(kb.keys.split("+"));
          if (controlModifiers.intersection(kbKeys).size > 0 !== ctrl)
            return false;
          if (shiftModifiers.intersection(kbKeys).size > 0 !== shift)
            return false;
          if (altModifiers.intersection(kbKeys).size > 0 !== alt) return false;
          if (metaModifiers.intersection(kbKeys).size > 0 !== meta)
            return false;
          return true;
        },
      );
