...
// except those exact file paths that have been explicitly drag-dropped into CutToTheChase or opened using the opener dialog
...
export default function useDirectoryAccess({
  setCustomOutDir,
}: {
  setCustomOutDir: (a: string | undefined) => void;
}) {
  const ensureAccessToSourceDir = useCallback(async (inputPath: string) => {
    ...
    for (;;) {
      ...
      if (
        (await checkDirWriteAccess(inputFileDir)) &&
        !simulateMasPermissionError
      )
        break;
      ...
      if (!masMode) {
        ...
        errorToast(
          i18n.t("You have no write access to the directory of this file"),
        );
  ...
  const ensureWritableOutDir = useCallback(
    async ({
      inputPath,
      outDir,
    }: {
      ...
      if (newCustomOutDir) {
        ...
        const customOutDirExists =
          (await mainApi.pathExists(newCustomOutDir)) &&
          (await lstat(newCustomOutDir)).isDirectory();
      ...
      const hasDirWriteAccess =
        effectiveOutDirPath != null &&
        (await checkDirWriteAccess(effectiveOutDirPath));
      if (!hasDirWriteAccess || simulateMasBuild) {
        if (masMode) {
          ...
          errorToast(
            i18n.t(
              "You have no write access to the directory of this file, please select a custom working dir",
            ),
          );
          setCustomOutDir(undefined);
          throw new DirectoryAccessDeclinedError();
...
export type EnsureWritableOutDir = ReturnType<
  typeof useDirectoryAccess
>["ensureWritableOutDir"];
