...
export async function loadMifiLink() {
  try {
    // In old versions: https://mifi.no/CutToTheChase/config.json
    return await ky("https://CutToTheChase.mifi.no/config.json").json();
    // return await ky('http://localhost:8080/CutToTheChase/config-dev.json').json();
  } catch (err) {
  ...
  try {
    ...
    if (err instanceof Error && !isMasBuild) {
      ...
      if (
        ...
        ["EPERM", "EACCES", "ENOENT"].includes(err.code)
      ) {
        onError({
          title: i18n.t("Fatal: FFmpeg not accessible"),
          message: [
            i18n.t(
              "Error code: {{errorCode}}. This could mean that anti-virus or something else is blocking the execution of FFmpeg. Make sure the following file exists and is executable:",
              { errorCode: err.code },
            ),
            "",
            getFfmpegPath(),
            "",
            i18n.t("Read more: {{url}}", {
              url: "https://github.com/mifi/lossless-cut/issues/1114",
            }),
          ].join("\n"),
        });
        return undefined;
