...
function App() {
  ...
  const [paramsByStreamId, setParamsByStreamId] = useState<ParamsByStreamId>(
    new Map(),
  );
    ...
    async ({
      ...
      try {
        ...
        let chaptersFromSegments: Awaited<
          ReturnType<typeof createChaptersFromSegments>
        >;
        if (segmentsToChapters) {
          const chapterNames = paths.map((path) => parsePath(path).name);
          chaptersFromSegments = await createChaptersFromSegments({
            segmentPaths: paths,
            chapterNames,
          });
        }
  ...
  const onExportConfirm = useCallback(async () => {
    ...
    try {
      ...
      if (willMerge) {
        ...
        // don't delete existing files that were not created by CutToTheChase now (due to overwrite disabled) https://github.com/mifi/lossless-cut/issues/2436
  ...
  const askStartTimeOffset = useCallback(async () => {
    const newStartTimeOffset = await promptTimecode({
      ...
      description: i18n.t(
        "Instead of video apparently starting at 0, you can offset by a specified value. This only applies to the preview inside CutToTheChase and does not modify the file in any way. (Useful for viewing/cutting videos according to timecodes)",
      ),
      inputPlaceholder: timecodePlaceholder,
      allowRelative: true,
    });
      ...
      await withErrorHandling(async () => {
        ...
        try {
          ...
          if (enableAskForFileOpenAction && inputOptionsKeys.length > 1)
            openFileResponse = await askForFileOpenAction(inputOptions);
  ...
  useEffect(() => {
    ...
    return () => {
      ipcActions.forEach(([key, action]) =>
        electron.ipcRenderer.off(key, action),
      );
    ...
    async (ev) => {
      ...
      await withErrorHandling(async () => {
        const filePaths = [...ev.dataTransfer.files].map((f) =>
          electron.webUtils.getPathForFile(f),
        );
                    ...
                    >
                      ...
                      {shouldShowPlaybackStreamSelector && (
                        <PlaybackStreamSelector
                          ...
                          activeSubtitleStreamIndex={activeSubtitleStreamIndex}
                          ...
                          onActiveVideoStreamChange={onActiveVideoStreamChange}
