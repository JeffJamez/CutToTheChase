...
type ActionsMap = Record<
  KeyboardAction,
  { name: string; category?: Category; before?: ReactNode }
>;
...
const CreateBinding = memo(
  ({
    ...
    return (
      <Dialog.Root
        open={isShown}
        onOpenChange={(v) => v === false && setCreatingBinding(undefined)}
      >
        <Dialog.Portal>
          <Dialog.Overlay />
          <Dialog.Content
          ...
          >
            ...
            <p>
              {t("Action:")} {actionsMap[action].name}{" "}
              <span style={{ color: "var(--gray-10)", marginLeft: ".5em" }}>
                <Action name={action} />
              </span>
            </p>
            ...
            <p>
              {t(
                "Please press your desired key combination. Make sure it doesn't conflict with any other binding or system hotkeys.",
              )}
            </p>
            ...
            <AnimatePresence>
              {isComboInvalid && (
                ...
                >
                  <Warning style={{ paddingBottom: "1em" }}>
            ...
            >
              {!keysDown.includes("esc") && (
                <Button
                  style={{ padding: ".2em .4em" }}
                  onClick={() => addKeyDown("esc")}
                >
                  ESC
                </Button>
              )}
              {keysDown.length > 0 && (
                <Button onClick={() => setKeysDown([])}>
                  <FaUndo
                    style={{
                      fontSize: ".8em",
                      verticalAlign: "middle",
            ...
            <Dialog.ButtonRow>
              <DialogButton
                disabled={fixedKeys.length === 0 || isComboInvalid}
                onClick={() =>
                  action != null && onNewKeyBindingConfirmed(action, keysDown)
                }
                primary
              >
                <FaSave
...
const KeyboardShortcuts = memo(
  ({
    ...
    const { actionsMap, extraLinesPerCategory } = useMemo(() => {
      ...
      const extraLinesPerCategory: Record<Category, ReactNode> = {
        [zoomOperationsCategory]: [
          ...
          <WheelModifier
            key="2"
            text={t("Seek one frame")}
            wheelText={t("Mouse scroll/wheel up/down")}
            modifier={mouseWheelFrameSeekModifierKey}
          />,

          <WheelModifier
            key="3"
            text={t("Seek one key frame")}
            wheelText={t("Mouse scroll/wheel up/down")}
            modifier={mouseWheelKeyframeSeekModifierKey}
          />,

          <WheelModifier
            key="4"
            text={t("Zoom in/out timeline")}
            wheelText={t("Mouse scroll/wheel up/down")}
            modifier={mouseWheelZoomModifierKey}
          />,
        ],
        [segmentsAndCutpointsCategory]: [
          <div key="1" style={{ ...rowStyle, alignItems: "center" }}>
            ...
            <div style={{ flexGrow: 1 }} />
            <kbd style={{ marginRight: ".7em" }}>
              {getModifier(segmentMouseModifierKey)}
            </kbd>
            <FaMouse style={{ marginRight: ".3em" }} />
    ...
    useEffect(() => {
      ...
      const validBindings = keyBindings.filter(
        ({ action }) => actionsMap[action],
      );

      if (validBindings.length !== keyBindings.length) {
        console.log(
          `Auto deleting ${keyBindings.length - validBindings.length} invalid key binding(s)`,
        );
    ...
    const actionEntries = useMemo(
      () =>
        (
          Object.entries(actionsMap) as unknown as [
            keyof typeof actionsMap,
            (typeof actionsMap)[keyof typeof actionsMap],
          ][]
        ).filter(
          ([key, { name, category }]) =>
            !isSearching ||
            ...
            (category != null &&
              category.toLowerCase().includes(searchQueryTrimmed)),
        ),
    ...
    const categoriesWithActions = useMemo(
      () =>
        Object.entries(groupBy(actionEntries, ([, { category }]) => category)),
      [actionEntries],
    );

    const onDeleteBindingClick = useCallback(
      async ({ action, keys }: { action: KeyboardAction; keys: string }) => {
    ...
    const onResetClick = useCallback(async () => {
      ...
      if (
        !(await confirmDialog({
          description: t(
            "Are you sure you want to reset all keyboard bindings?",
          ),
        }))
      )
        return;
    ...
    const onNewKeyBindingConfirmed = useCallback(
      async (action: KeyboardAction, keys: string[]) => {
        const keysStr = stringifyKeys(keys);
        console.log("New key binding", action, keysStr);

        const duplicate = keyBindings.find(
          (existingBinding) => existingBinding.keys === keysStr,
        );
        ...
        if (duplicate) {
          const isConfirmed = await confirmDialog({
            ...
            description: t(
              'Combination is already bound to "{{alreadyBoundKey}}". Do you want to replace the existing binding?',
              { alreadyBoundKey: actionsMap[duplicate.action]?.name },
            ),
          });
        ...
        setKeyBindings((existingBindings) => {
          ...
          const filtered = !shouldReplaceDuplicate
            ? existingBindings
            : existingBindings.filter(
                (existing) => existing.keys !== shouldReplaceDuplicate.keys,
              );
    ...
    return (
      <>
          ...
          {categoriesWithActions.map(([category, actionsInCategory]) => (
            <div key={category}>
              ...
              {actionsInCategory.map(([action, actionObj]) => {
                ...
                const bindingsForThisAction = keyBindings.filter(
                ...
                return (
                  <div key={action} style={rowStyle}>
                    <div>
                      ...
                      <span
                        title={action}
                        style={{ marginRight: ".5em", opacity: 0.9 }}
                      >
                        {actionName}
                      </span>
                      <div
                        style={{ fontSize: ".8em", opacity: 0.3 }}
                        title={t("API action name: {{action}}", { action })}
                      >
                        {action}{" "}
                        <Button onClick={() => onTriggerActionClick(action)}>
                          <FaHammer />
                        </Button>
                      </div>
                    </div>
                    ...
                    >
                      {bindingsForThisAction.map(({ keys }) => (
                        ...
                        >
                          ...
                          <Button
                            title={t("Remove key binding")}
                            onClick={() =>
                              onDeleteBindingClick({ action, keys })
                            }
                      ...
                      {bindingsForThisAction.length === 0 && (
                        <span style={{ opacity: 0.8, fontSize: ".8em" }}>
                          {t("No binding")}
                        </span>
                      )}
                    </div>
                    ...
                    <Button
                      ...
                      onClick={() => onAddBindingClick(action)}
        ...
        {!isSearching && (
          <Button onClick={onResetClick} style={{ marginBottom: "1em" }}>
            <FaUndo
              style={{
                marginRight: ".3em",
                verticalAlign: "middle",
        ...
        <CreateBinding
          actionsMap={actionsMap}
          action={creatingBinding}
          setCreatingBinding={setCreatingBinding}
          onNewKeyBindingConfirmed={onNewKeyBindingConfirmed}
        />
      </>
    );
  },
...
function KeyboardShortcutsDialog({
  ...
  const handleTriggerActionClick = useCallback(
      ...
      <Dialog.Portal>
        ...
        <Dialog.Content style={{ width: "40em" }} aria-describedby={undefined}>
          ...
          <KeyboardShortcuts
            ...
            onTriggerActionClick={handleTriggerActionClick}
          />
