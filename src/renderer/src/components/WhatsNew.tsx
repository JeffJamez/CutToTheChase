  ...
  const matchingVersions = useMemo(
    () =>
      versions
        .filter(
          ({ version }) =>
            semver.gt(version, initialLastAppVersion) &&
            semver.lte(version, appVersion),
        )
        .sort(({ version: a }, { version: b }) => semver.compare(b, a)),
    [initialLastAppVersion],
  );
    ...
    <Dialog.Root defaultOpen>
      <Dialog.Portal>
        ...
        <Dialog.Content style={{ width: "60em" }} aria-describedby={undefined}>
          <Dialog.Title>{t("What's new in CutToTheChase?")}</Dialog.Title>

          <div>
            {matchingVersions.map(({ version, highlightsMd }, i) => {
              // +1 because reverse order
              const prevVersion =
                matchingVersions[i + 1]?.version ?? initialLastAppVersion;
              ...
              return (
                <div key={version} className={styles["version"]}>
                  ...
                  >
                    ...
                    <Button
                      onClick={() => shell.openExternal(getReleaseUrl(version))}
                    >
                      <FaFile style={{ verticalAlign: "middle" }} />{" "}
                      {t("All release notes")}
                    </Button>
                    <Button
                      onClick={() =>
                        shell.openExternal(
                          compareReleasesUrl(prevVersion, version),
                        )
                      }
                    >
