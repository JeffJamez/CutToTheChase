...
function ExportConfirm({
  ...
  const onTracksHelpPress = useCallback(() => {
    showHelpText({
      text: i18n.t(
        "Not all formats support all track types, and CutToTheChase is unable to properly cut some track types, so you may have to sacrifice some tracks by disabling them in order to get correct result.",
      ),
    });
  }, [showHelpText]);
  ...
  const onAvoidNegativeTsHelpPress = useCallback(() => {
    ...
    const texts = {
      ...
      make_zero: i18n.t(
        "Shift timestamps so that the first timestamp is 0. (CutToTheChase default)",
      ),
