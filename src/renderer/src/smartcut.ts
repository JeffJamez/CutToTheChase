...
export async function needsSmartCut({
  path,
  desiredCutFrom,
  videoStream,
}: {
  ...
  const readKeyframes = async (window: number) =>
    readKeyframesAroundTime({
      filePath: path,
  ...
  if (keyframeAtExactTime) {
    ...
    return {
      CutToTheChaseFrom: keyframeAtExactTime.time,
  ...
  if (nextKeyframe == null)
    throw new UserFacingError(
      i18n.t("Cannot find any keyframe after the desired start cut point"),
    );
  ...
  return {
    CutToTheChaseFrom: nextKeyframe.time,
    segmentNeedsSmartCut: true,
  };
}

// eslint-disable-next-line import/prefer-default-export
export async function getCodecParams({
  path,
  fileDuration,
  streams,
}: {
