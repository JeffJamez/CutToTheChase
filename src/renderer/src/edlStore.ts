...
async function loadCsv(
  path: string,
  parseTimeFn: (a: string) => number | undefined,
) {
...
export async function saveCsvFrames({
  path,
  cutSegments,
  getFrameCount,
}: {
...
export async function saveLlcProject({
  savePath,
  mediaFilePath,
  cutSegments,
}: {
...
export async function loadLlcProject(path: string) {
  ...
  console.log(
    `Loaded LLC project v${project.version}, mediaFileName: ${project.mediaFileName}, ${project.cutSegments.length} segments`,
  );
...
export async function readEdlFile({
  type,
  path,
  fps,
}: {
...
export async function askForEdlImport({
  type,
  fps,
  fileDuration,
}: {
  ...
  if (type === "csv" || type === "csv-frames")
    filters = [{ name: i18n.t("CSV files"), extensions: ["csv"] }];
  else if (type === "xmeml")
    filters = [{ name: i18n.t("XML files"), extensions: ["xml"] }];
  else if (type === "fcpxml")
    filters = [{ name: i18n.t("FCPXML files"), extensions: ["fcpxml"] }];
  else if (type === "cue")
    filters = [{ name: i18n.t("CUE files"), extensions: ["cue"] }];
  else if (type === "pbf")
    filters = [{ name: i18n.t("PBF files"), extensions: ["pbf"] }];
  else if (type === "edl")
    filters = [{ name: i18n.t("EDL"), extensions: ["*"] }];
  else if (type === "dv-analyzer-summary-txt")
    filters = [
      { name: i18n.t("DV Analyzer Summary.txt"), extensions: ["txt"] },
    ];
  else if (type === "srt")
    filters = [{ name: i18n.t("Subtitles (SRT)"), extensions: ["srt"] }];
  else if (type === "llc")
    filters = [{ name: i18n.t("CutToTheChase project"), extensions: ["llc"] }];
...
export async function exportEdlFile({
  type,
  cutSegments,
  customOutDir,
  filePath,
  ...
  if (type === "csv") {
    ...
    ext = "llc";
    filters = [
      { name: i18n.t("CutToTheChase project"), extensions: [ext, "llc"] },
    ];
  }

  const defaultPath = getOutPath({
    filePath,
    customOutDir,
  ...
  if (type === "csv") await saveCsv(savePath, cutSegments);
    ...
    await saveCsvFrames({ path: savePath, cutSegments, getFrameCount });
  else if (type === "llc")
    await saveLlcProject({ savePath, mediaFilePath: filePath, cutSegments });
