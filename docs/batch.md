...
I get a lot of questions about whether CutToTheChase can help automate the **exact same operation on X number of files**. For example given a folder of 100 files, cut off 10 seconds from the beginning of every file, or split each file into 30 second files. CutToTheChase was not designed to be a fully auomated batch processing toolkit and generally cannot not do these things, however the good news is that often it's not very hard to automate with a simple script.
...
### Action from CutToTheChase

If there's a particular operation from CutToTheChase you want to automate, you can find the command from the "Last FFmpeg commands" page. Then copy it and paste it into your AI prompt. For example:
