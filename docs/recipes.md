...
3. Open the extracted video stream in CutToTheChase. The open your encoded audio file and select "Include all tracks from the new file".
4. Export.

## Advanced multi-step workflows 🔢

Use CutToTheChase in multiple passes in order to achieve separate trimming of individual tracks:
...
1. Drag-drop source files (from the same camera/source) into CutToTheChase to open them in the batch list.
...
9. You will have a folder with all the ordered segments, now drag drop them into CutToTheChase and sort by name in the batch list.
10. Merge
...
2. _(Only once:)_ Click "Filter tracks" (top left), enter the expression: `track.codec_type === 'video' || track.codec_type === 'audio'`
