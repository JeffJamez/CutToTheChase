...
const CutToTheChaseExePath = process.argv[2];
assert(CutToTheChaseExePath);
...
const CutToTheChaseArgs = [
  ...(platform === "linux" ? ["--no-sandbox"] : []),
  "--http-api",
  String(port),
];
const ps = execa(CutToTheChaseExePath, CutToTheChaseArgs, {
  forceKillAfterDelay: 10000,
});

console.log("Started", CutToTheChaseExePath);
...
async function captureScreenshot(outPath: string) {
  ...
  await execa(
    ...
    [
      ...(platform === "darwin"
        ? ["-r", "30", "-pix_fmt", "uyvy422", "-f", "avfoundation"]
        : []),
