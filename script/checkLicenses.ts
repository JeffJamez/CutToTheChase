...
const { stdout } = await execa("yarn", ["licenses", "list", "-R", "--json"], {
  encoding: "utf8",
});

const licensesJson: { value: string; children: Record<string, unknown> }[] =
...
const safeLicenses: Record<string, true | string[]> = {
  ...
  // which is OK because CutToTheChase is also GPL-2.0-only
...
const unsafeLicenses = licensesJson.flatMap((l) => {
  const checkLicense = (license: string) => {
    ...
    return Object.keys(l.children).every((pkg) =>
      safeLicense.some((safeLicensePackage) =>
        pkg.startsWith(safeLicensePackage),
      ),
    );
  };
